...
export async function createEventRelativeAction(formData: FormData) {
  ...
  const from = (formData.get("from")?.toString() ?? "event") as "event" | "participants";
  ...
  const baseTarget =
    from === "participants" ? `/event/${slug}/participants` : `/event/${slug}#participants`;
  ...
  if (hasProfileChoice && !hasNewData) {
    ...
    if (!profile) {
      redirect(`/event/${slug}/participants/add-relative?error=forbidden&from=${from}`);
    }
    ...
    if (!firstNameToUse) {
      redirect(`/event/${slug}/participants/add-relative?error=invalid&from=${from}`);
    }
  ...
  await prisma.$transaction(async (tx) => {
    const event = await tx.event.findUnique({
      ...
      select: {
        ...
        giftMode: true,
        modules: {
          where: { key: "GIFTS" }, // keep as string to avoid importing EventModuleKey if you want
          select: { enabled: true },
          take: 1,
        },
      },
