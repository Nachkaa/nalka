...
export default async function AddRelativePage({ params, searchParams }: Props) {
    ...
    return (
        <main className="mx-auto flex w-full max-w-md flex-col gap-6 px-4 py-6">
            ...
            <section className="space-y-4 rounded-xl border bg-background px-4 py-5">
                <div className="space-y-1">
                    <h1 className="text-base font-semibold">Ajouter un proche</h1>
                    <p className="text-sm text-muted-foreground">
                        Crée un participant sans compte (enfant, parent, etc.). Tout le
                        monde pourra voir et réserver sur sa liste de cadeaux.
                    </p>
                </div>
                    ...
                    {existingProfiles.length > 0 && (
                        <fieldset className="space-y-2 rounded-lg border bg-muted/40 px-3 py-3">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Sélectionne un proche déjà enregistré, ou laisse vide pour en
                                créer un nouveau juste en dessous.
                            </p>
                            <div className="mt-2 flex flex-wrap gap-2">
                                {existingProfiles.map((p) => (
                                    <label key={p.id} className="cursor-pointer">
                                        ...
                                        />
                                        <div className="flex items-center gap-2 rounded-full border border-dashed px-3 py-1 text-xs transition
                                    peer-checked:border-[var(--primary)]
                                    peer-checked:bg-[color-mix(in_oklch,var(--primary),white_90%)]">
