// src/app/(app)/event/[slug]/_components/header/RsvpBar.tsx
"use client";
...
function statusLabel(s: EventRsvpStatus) {
  return OPTIONS.find((o) => o.value === s)?.label ?? "Indiquez si vous venez";
}
...
function statusPillClasses(s: EventRsvpStatus) {
  if (s === EventRsvpStatus.GOING)
    return "bg-[color:var(--success-light)] text-[color:var(--success-dark)] border-[color:var(--success-dark)]";
  if (s === EventRsvpStatus.MAYBE)
...
export function RsvpBar({
  ...
  const handleSelect = (next: EventRsvpStatus) => {
    ...
    startTransition(async () => {
      const result = await updateRsvp({ eventId, status: next });
