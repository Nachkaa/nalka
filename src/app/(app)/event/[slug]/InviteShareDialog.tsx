...
export function InviteShareDialog({ eventRef }: { eventRef: string }) {
  ...
  const hint = useMemo(() => {
    ...
    if (!expiresAt) {
      return `Jusqu’à ${n} ${people} • Lien valable environ 7 jours.`;
    }

    // Si un jour tu passes expiresAt depuis le serveur, force une locale fixe
    const formatted = expiresAt.toLocaleString("fr-FR", {
      dateStyle: "short",
      timeStyle: "short",
    });

    return `Jusqu’à ${n} ${people} • Expire le ${formatted}.`;
  }, [remaining, expiresAt]);
