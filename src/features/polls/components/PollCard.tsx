...
import { setEventDateBySlug, setEventLocationBySlug } from "@/features/events/server/event-details";
import { EventPollStatus, EventPollType } from "@prisma/client";
import { BarChart3, Plus } from "lucide-react";
import { useRouter } from "next/navigation";
import { useMemo, useState, useTransition } from "react";

import { getDefineLabel, getRecommendedOptionId, isRecommendationStrong } from "../lib/poll-utils";
import { applyPollOptionToEvent, closePollById, togglePollVote } from "../server/mutations";
...
export function PollCard({
  ...
  const recommendedOptionId = useMemo(
    () => getRecommendedOptionId(poll.options.map((option) => ({ id: option.id, count: option.count }))),
    [poll.options],
  );
  const recommendedCount =
    poll.options.find((option) => option.id === recommendedOptionId)?.count ?? 0;
  const strong = isRecommendationStrong(
    poll.options.map((option) => ({ count: option.count })),
    recommendedCount,
  );
  ...
  const onApplyManual = (value: string) => {
    ...
    startTransition(async () => {
      if (poll.type === EventPollType.SCHEDULE) {
        await setEventDateBySlug(slug, next);
      } else if (poll.type === EventPollType.LOCATION) {
        await setEventLocationBySlug(slug, next);
      }
