...
import { launchDraw, setSecretSantaBudget } from "../server/mutations";
import type { SecretSantaMeView, SecretSantaModuleProps } from "../types";

type SecretSantaScreenProps = SecretSantaModuleProps & {
  meEndpoint: string;
};
...
async function fetchMyTarget(meEndpoint: string): Promise<SecretSantaMeView | null> {
  try {
    const response = await fetch(meEndpoint, { cache: "no-store" });
    if (!response.ok) return null;
    return (await response.json()) as SecretSantaMeView;
  } catch {
    return null;
  }
}

export function SecretSantaScreen({
  eventId,
  slug,
  isAdmin,
  membersCount,
  ...
  isHandmadeOk,
  meEndpoint,
}: SecretSantaScreenProps) {
  const router = useRouter();
  const [isRefreshing, startTransition] = useTransition();
  const [isBudgetSaving, startBudgetTransition] = useTransition();
  const [target, setTarget] = useState<SecretSantaMeView | null>(null);
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
            ...
            <form
              action={async (formData) => {
                ...
                startTransition(async () => {
                  const fresh = await fetchMyTarget(meEndpoint);
                  if (fresh) {
                    setTarget(fresh);
                    setHasDraw(true);
                  }
          ...
          <div className="mt-4 space-y-3 rounded-xl border p-3">
            ...
            {budgetError ? <p className="text-xs text-red-600">{budgetError}</p> : null}
          </div>
